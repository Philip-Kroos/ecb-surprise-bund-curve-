"""Loading and cleaning of the Euro Area Monetary Policy Event-Study Database.

The single non-trivial step here is date parsing. The distributed workbook
stores the event date as a genuine Excel date for early observations and as a
``dd/mm/yyyy`` **text string** for later ones. Naive parsers apply US month-first
conventions to the text rows and silently transpose day and month whenever both
are smaller than thirteen. In the current vintage this corrupts roughly forty
percent of event dates, which in turn corrupts the regime assignment that the
entire paper rests on. We therefore read the raw cells, branch on cell type and
verify the result against the known calendar of Governing Council meetings.
"""
