...
\author{Philip Kroos\thanks{Contact details. Replication code and data
are available at \texttt{github.com/philipkroos/ecb-surprises-bund-curve}. All errors
are my own.}}
\date{\today}
...
balance sheet normalisation. It rotated. Balance-sheet surprises pass through to
Bund yields with a coefficient near unity at every maturity and in both regimes.
Rate and guidance surprises, which during the expansion propagated to the long
end and inverted it, are now absorbed at the front and transmitted into curve
shape. Alongside this, the incidence of information shocks fell by two thirds and
...
\caption{Factor loadings on OIS surprises}
\label{tab:loadings}
\input{../output/tables/t1_loadings.tex}
...
\caption{Baseline curve responses, pooled sample}
\label{tab:baseline}
\input{../output/tables/t3_baseline.tex}
\begin{minipage}{\textwidth}\footnotesize\vspace{4pt}
...
\begin{table}[htbp]\centering\footnotesize
\caption{Regime interactions: the central result}
\label{tab:interaction}
\input{../output/tables/t4_interaction.tex}
\begin{minipage}{\textwidth}\footnotesize\vspace{4pt}
...
\begin{table}[htbp]\centering\footnotesize
\caption{Joint stability tests}
\label{tab:stability}
\input{../output/tables/t12_stability.tex}
\begin{minipage}{\textwidth}\footnotesize\vspace{4pt}
...
\begin{table}[htbp]\centering\footnotesize
\caption{Shock classification by regime}
\label{tab:shocks}
\input{../output/tables/t5_shock_types.tex}
\end{table}

\begin{table}[htbp]\centering\footnotesize
\caption{Country panel: differential transmission to the periphery}
\label{tab:panel}
\input{../output/tables/t7_panel_fragmentation.tex}
\begin{minipage}{\textwidth}\footnotesize\vspace{4pt}
...
\begin{table}[htbp]\centering\footnotesize
\caption{Sovereign spreads over Bunds}
\label{tab:spreads}
\input{../output/tables/t8_spreads.tex}
\end{table}

\begin{table}[htbp]\centering\footnotesize
\caption{Scenario calibration}
\label{tab:scenarios}
\input{../output/tables/t10_scenarios.tex}
\begin{minipage}{\textwidth}\footnotesize\vspace{4pt}
...
\begin{table}[htbp]\centering\footnotesize
\caption{Robustness of the ten-year path interaction}
\label{tab:robustness}
\input{../output/tables/t9_robustness_summary.tex}
\end{table}
...
\caption{Curve response by regime, with 90 percent confidence bands. The middle
panel contains the paper's central finding: the negative long-end response to
path surprises present during the expansion is absent afterwards.}
\label{fig:curve}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\textwidth]{../output/figures/fig4_rolling.pdf}
\caption{Rolling path coefficients over 25-meeting windows with 90 percent bands,
estimated on the sample including the transition. The transition is continuous
...
\caption{Interest rate surprise against equity response. Points in the
co-movement quadrants are classified as information shocks.}
\label{fig:information}
\end{figure}

\begin{table}[htbp]\centering\footnotesize
\caption{Appendix A1: event dates mis-assigned by a month-first parser}
\input{../output/tables/a1_date_audit.tex}
\end{table}
